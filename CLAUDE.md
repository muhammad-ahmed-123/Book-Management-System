...
"Exhaustive" means exhaustive coverage of code this app actually wrote — not of Rails/Ruby's built-in behavior. Don't add a spec whose assertion would still pass if the app-specific code were deleted and only bare framework defaults remained (e.g. a presence test on a plain `belongs_to` with no explicit `presence: true` added, or an association test that only confirms `belongs_to`/`has_many` returns the related record with no custom `through`/`source`/scope). Do write specs for validations/scopes/formats the app defined itself, deliberate business-rule guards (or their deliberate absence — e.g. `Favourite` intentionally allows self-favouriting where `Review` blocks self-review), `dependent: :destroy` cascades, and DB-level constraints that backstop an AR validation (e.g. a unique index guarding a TOCTOU race).
...
- `RegistrationsController` (`resource :registration`, singular — `new`/`create` only): self-service sign-up. Mirrors `SessionsController`'s public-action/rate-limiting shape, but additionally redirects an already-authenticated visitor away from the form (unlike sign-in, re-registering while logged in has no legitimate use case). On success it calls the same `start_new_session_for` helper `SessionsController#create` uses, so sign-up auto-logs-in. `User` has `validates :email_address, presence: true, uniqueness: true` and `validates :password, length: { minimum: 8 }` specifically to make these public-facing forms produce friendly validation errors instead of raw DB exceptions. There is no "edit profile without touching the password" feature, so the length validation is intentionally strict (no `allow_nil`) — every save must carry a real, minimum-length password. `has_secure_password`'s setter no-ops on `""` (leaving `password`/`password_digest` untouched rather than nil), but `PasswordsController#update` calls `@user.update(...)` on a freshly-loaded record, so `password` reads back as `nil` and the length validator still rejects a blank submission — see `spec/requests/passwords_spec.rb`.
